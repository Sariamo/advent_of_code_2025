...
for coord1 in coords:
    for coord2 in coords:
        ...
        connections_plat = [x for _, x in sorted(zip(distances, connections_plat))][:1000]
        distances = sorted(distances)[:1000]
...
lengths = sorted([len(c) for c in connections], reverse=True)
